...
| 인코딩| 특징 식별| 디코딩 방법|
|------|---------|---------|
| Base64 | `=` 또는 `==`로 끝나는 문자 집합 A-Za-z0-9+/| `base64 -d` / 사이버셰프|
| Base32 | 대문자 + 2-7, `=`로 끝남| CyberChef |
...
| JWT | `xxxxx.yyyyy.zzzzz`(세 개의 세그먼트 Base64URL)| jwt.io/CyberChef|
| Brainfuck | `><+-.,[]` 8자만 가능| 온라인 인터프리터|
...
| `0x61707865` ("엑스파")| ChaCha20/Salsa20 |
| `0xC6EF3720` | XTEA |
...
| Feistel 구조(좌우 교환)| DES/Blowfish/TEA |
| 큰 수 곱셈/모듈러 지수화| RSA |
| 타원 곡선 점 작업| ECDSA/ECDH |
| 고정된 64라운드 루프| TEA/XTEA |
| 32라운드 + 델타 상수| XTEA |
...
1. 길이와 문자 집합을 확인
   ...
   - 끝에 = 있음 → Base64
   - 점으로 구분된 세 구간 → JWT
   - 32/40/64자 hex → 해시(MD5/SHA1/SHA256)
